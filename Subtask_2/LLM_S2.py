...
def data_to_vec(model, tokenizer, df_path, save_path, cls_pos=0):
  ...
  df_path: str or Path
    Ruta del dataset en formato csv
...
def main():
  parser = argparse.ArgumentParser(description='Embeddings cls.')
  ...
  models=["andricValdez/bert-base-multilingual-cased-finetuned-autext24-subtask2",
  "andricValdez/multilingual-e5-large-finetuned-autext24-subtask2",
  "andricValdez/xlm-roberta-base-finetuned-autext24-subtask2"]
  ...
  for i in range(len(models_l)):
    ...
    if data==0:
      data_to_vec(
        ...
        df_path=df_path,
        save_path=save_path + "train_subtask2" + model_names[i] + ".csv",
        cls_pos=cls_pos
      )
    else:
      data_to_vec(
        ...
        tokenizer=tokenizers[i],
        df_path=df_path,
        save_path=save_path + "test_subtask2" + model_names[i] + ".csv",
        cls_pos=cls_pos
      )
